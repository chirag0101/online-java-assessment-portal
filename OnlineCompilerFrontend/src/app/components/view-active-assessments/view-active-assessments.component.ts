...
import { AssessmentService, ActiveAssessment, ActiveAssessmentsResponse, ExtendAssessment, UrlRes } from '../../services/assessment.service';
...
export class ViewActiveAssessmentsComponent implements OnInit {
  ...
  fetchActiveAssessments(): void {
    ...
    if(sessionStorage.getItem("isAdmin")==="false"){
      this.assessmentService.getActiveAssessmentsForCurrentAdmin(sessionStorage.getItem("adminId")??'').subscribe({
        next: (data: ActiveAssessmentsResponse) => {
            if (data.statusMessage === 'SUCCESS' && data.response) {
              ...
              this.errorMessage = data.statusMessage || 'Failed to load active assessments.';
            }
...
copyUrl(candidateId: string): void {
  ...
  if (finalCandidateId!==null) {
    ...
    this.assessmentService.getUrlByCandidateId(finalCandidateId).subscribe({
      ...
      error: (err) => {
        ...
        this.copySuccess = false;
        alert('Error fetching assessment URL. Please check your connection or try again later.');
      }
    });
  } else {
    this.copySuccess = false;
  ...
  extendAssessment(candidateId: string): void {
    ...
    dialogRef.afterClosed().subscribe(minutes => {
      if (minutes !== null && minutes > 0) {
        console.log(`Extending assessment for ${candidateId} by ${minutes} minutes.`);
  ...
  endAssessment(candidateId: string): void {
    ...
    this.openConfirmationDialogBox(2).then((flag: boolean) => {
      if (flag) { 
        this.assessmentService.endAssessment(candidateId).subscribe({
          next: () => {
            this.fetchActiveAssessments();
          },
          error: (err) => {
